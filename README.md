...
tool 0.1.1  run 2026-06-01  scale shipgrade-1
...
shipgrade ships a GitHub Action that runs the same scan in a pull request, uploads SARIF to the
Security tab, and fails the check on the grade you choose. The action installs `uv` and shipgrade
for you, so the workflow needs only a checkout:
...
jobs:
  audit:
    ...
    steps:
      ...
      - uses: RivetaLabs/shipgrade@v0.1.1
        with:
          config: shipgrade.yaml
          fail-on: high
```

Your `shipgrade.yaml` must list `sarif` in its `outputs` for the Security-tab upload, and `json`
to populate the `score`, `grade`, and `findings-count` action outputs. The action fetches shipgrade
from PyPI at run time, so the runner needs network access. Pin `@v0.1.1` for a fixed release, or
`@v1` to track the latest v1.x. Every input is optional and documented in [`action.yml`](action.yml).
...
The OpenSSF Scorecard checks this repo's supply-chain posture on every push to `main` and
weekly. The score is generated by the OpenSSF, never hand-written:

[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/RivetaLabs/shipgrade/badge)](https://securityscorecards.dev/viewer/?uri=github.com/RivetaLabs/shipgrade)
